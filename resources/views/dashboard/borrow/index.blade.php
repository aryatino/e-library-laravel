            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                    ...
                    <td class="px-6 py-4 capitalize">
                        ...
                        <p class="bg-yellow-300 text-black p-1 text-center rounded">{{ $borrow->status }}</p>
                        @elseif ($borrow->status == "dipinjam")
                        <p class="bg-green-300 text-black p-1 text-center rounded">{{ $borrow->status }}</p>   
                        @elseif ($borrow->status == "dikembalikan")
                        <p class="bg-blue-300 text-black p-1 text-center rounded">{{ $borrow->status }}</p>   
                        @elseif ($borrow->status == "ditolak")
                        <p class="bg-red-300 text-black p-1 text-center rounded">{{ $borrow->status }}</p>   
                        @endif
                    </td>
                    <td class="px-6 py-4 flex gap-2">
                        @if ($borrow->status == 'diajukan' || $borrow->status == 'dipinjam')
                             <div class="text-yellow-500 hover:text-yellow-600">
                            <a href="/dashboard/borrow/{{ $borrow->id }}/edit"><i class="fa-sharp fa-solid fa-pen-to-square"></i> Edit</a>
                        </div>
                ...
                @else
                  <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                    <td colspan="7" class="p-4 text-center text-white font-semibold">Tidak Ada Data Peminjaman</td>
                  </tr>
                @endif
           </tbody>
        </table>
